...
fact_input_dim = 4
# Hidden layer optimal size
h_l_o_s = 100


# ----------------------------------
# Not possible to train with all "800" programs -> out of memory => half of it works
# ----------------------------------
class C_C_Ff_Final_TwoAround:
    ...
    def prepare_data(self, token_lists):
        ...
        h_l_o_s = len(all_token_strings)
        print("Unique tokens: " + str(h_l_o_s))
    ...
    def create_network(self):
        self.net = tflearn.input_data(shape=[None, len(self.string_to_number) * fact_input_dim])
        self.net = tflearn.fully_connected(self.net, h_l_o_s * 2)
        self.net = tflearn.fully_connected(self.net, h_l_o_s)
        self.net = tflearn.fully_connected(self.net, h_l_o_s)
        self.net = tflearn.fully_connected(self.net, len(self.string_to_number) * 3, activation='softmax')
        # Stochastic Gradient Descent as optimizer (worse results)
        # Binary instead of categorical crossentropy as loss function (worse results)
        # Loss fct 0.001 -> 5 (worse results)
